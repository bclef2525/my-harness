...
description: "스테이지된 변경(git diff --cached)과 최근 커밋 로그를 읽고 Conventional Commits 형식의 커밋메시지 초안을 작성한다. '커밋 메시지', 'commit message' 같은 자연어 요청시 사용"
model: sonnet
...
- 제목은 72자 이하, 명령형 현재시제. `type(scope):subject` 패턴
- 스타일이 혼재하면 최근 10개 중 다수결을 따른다.
- diff에 없는 변경을 제목이나 본문에 넣지 않는다 - 추측 금지.
- 본문은 3줄 이내, 변경 이유 중심.
- 최근 커밋 10개 이하로 존재한다면 10개 이하의 존재하는 커밋들을 참조하고 기존 커밋이 없는 경우 참조없이 작업원칙에 따라 생성한다.
