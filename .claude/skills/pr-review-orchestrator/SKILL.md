...
- `_workaspace/pr-review/team-log.jsonl`
...
4. `_workspace/pr-review/team-log.jsonl`에 팀 생성, 작업배치, 종료 시각을 append-only로 남긴다.

## 실패 처리

- 팀원 1명 실패: 같은 역할을 한번 재시도 한다.
...
- TeamDelete 실패: 숨기지 않고 수동 정리 필요 항목과 수동정리 방법을 보고한다.
